 ...
 * - checks whether the signature section of the JWT is a valid signature for the header and payload sections for this public key.
 */
abstract class JwtSignatureValidator implements Validator<Token> {
    ...
    public ValidationResult validate(Token token) {
        ...
        try {
            ...
            return createInvalid("Token signature can not be validated because JWKS could not be fetched: {}", e.getMessage());
    ...
    /**
     * Service-specific implementation for the retrieval of the public key, e.g. via URL from JKU header (XSUAA) or OIDC .well-known endpoint (IAS)
     */
    protected abstract PublicKey getPublicKey(Token token, JwtSignatureAlgorithm algorithm) throws OAuth2ServiceException, InvalidKeySpecException, NoSuchAlgorithmException;
    ...
    protected ValidationResult validateSignature(Token token, PublicKey publicKey, JwtSignatureAlgorithm algorithm) {
        ...
        try {
            ...
            return createInvalid("Token signature can not be validated because implementation of algorithm could not be found: {}", e.getMessage());
        }
        ...
        try {
            ...
            return createInvalid("Signature of Jwt Token is not valid: the identity provided by the JSON Web Token Key can not be trusted (Signature: {}).", signature);
        } catch (Exception e) {
