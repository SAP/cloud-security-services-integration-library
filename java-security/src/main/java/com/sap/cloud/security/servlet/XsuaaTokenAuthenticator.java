...
public class XsuaaTokenAuthenticator extends AbstractTokenAuthenticator {
                ...
                try {
                    ...
                    if (isIasXsuaaXchangeEnabled() && !isXsuaaToken(token)) {
                        token = new XsuaaToken(Objects.requireNonNull(
                                exchangeBroker.doIasToXsuaaXchange(httpClient, token, serviceConfiguration), "IasXsuaaExchangeBroker is not provided"
                        ));
                    }
    ...
    private TokenAuthenticationResult tokenValidationResult(Token token){
    ...
    private boolean isXsuaaToken(Token token) {
        try {
            ...
            String externalAttributeValue = Objects.requireNonNull(externalAttributeClaim, "ext_attr claim is missing")
                    .getAsString(EXTERNAL_ATTRIBUTE_ENHANCER);
            ...
            logger.debug("Token is not Xsuaa token: {}", e.getMessage());
            return false;
        }
    }
