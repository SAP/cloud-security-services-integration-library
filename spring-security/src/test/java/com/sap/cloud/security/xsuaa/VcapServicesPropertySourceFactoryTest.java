...
@ContextConfiguration(classes = { VcapPropertyTestBean.class })
public class VcapServicesPropertySourceFactoryTest {

	@ClassRule
	public static InjectVcapServiceRule server = new InjectVcapServiceRule();

	@Autowired
	VcapPropertyTestBean exampleBean;
...
@Configuration
@PropertySource(factory = VcapServicesPropertySourceFactory.class, value = { "" })
class VcapPropertyTestBean {

	@Value("${vcap.services.example-xsuaa.credentials.url:}")
	private String xsuaaUrl;

	@Value("${xsuaa.unknown:}")
	private String unknown;

	public VcapPropertyTestBean() {
	}
