...
class XsuaaServiceConfigurationsTest {
    ...
    void configuresXsuaaServiceConfigurations() {
        runner.withUserConfiguration(EnablePropertiesConfiguration.class)
                ...
                .run(context -> {
                    assertEquals("cid1", context.getBean(XsuaaServiceConfigurations.class).getConfigurations().get(0).getClientId());
                    assertEquals("cid2", context.getBean(XsuaaServiceConfigurations.class).getConfigurations().get(1).getClientId());
                });
    }
}
