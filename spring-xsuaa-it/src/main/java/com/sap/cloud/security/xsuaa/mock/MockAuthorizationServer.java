		...
		Dispatcher dispatcher = new Dispatcher() {
			...
			public MockResponse dispatch(RecordedRequest request) throws InterruptedException {
				...
				if (request.getPath().equals("/oauth/token")) {
					...
					if ("basic c2ItamF2YS1oZWxsby13b3JsZDpteXNlY3JldC1iYXNpYw==".equalsIgnoreCase(
							request.getHeader("authorization")) && "POST".equals(request.getMethod()) && body.contains("username=basic.user") && body.contains("password=basic.password")) {
				
						try {
							return new MockResponse().setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
							.setResponseCode(200).setBody(String.format("{\"access_token\": \"%s\"}", JWTUtil.createJWT("/password.txt","testdomain")));
						} catch (Exception e) {
							e.printStackTrace();
							getResponse("/mock/500.txt", 500);
						}
					}
					if ("basic c2ItamF2YS1oZWxsby13b3JsZDpiYXNpYy5jbGllbnRzZWNyZXQ=".equalsIgnoreCase(
							request.getHeader("authorization")) && "POST".equals(request.getMethod()) && body.contains("grant_type=client_credentials")) {
				
						try {
							return new MockResponse().setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
							.setResponseCode(200).setBody(String.format("{\"access_token\": \"%s\"}", JWTUtil.createJWT("/cc.txt","testdomain")));
						} catch (Exception e) {
							e.printStackTrace();
							getResponse("/mock/500.txt", 500);
						}
