...
import com.sap.cloud.security.client.DefaultServiceManagerService;
import com.sap.cloud.security.client.ServiceManagerService;
...
public class K8sEnvironment implements Environment {
	...
	private final Map<Service, Map<String, OAuth2ServiceConfiguration>> serviceConfigurations = new HashMap<>(); // use
	...
	private static Map<Service, Map<String, OAuth2ServiceConfiguration>> getServiceConfigurations() {
		if (instance.serviceConfigurations.isEmpty()) {
			if (instance.serviceManagerConfigurations != null) {
				serviceManagerService = new DefaultServiceManagerService(instance.serviceManagerConfigurations,
						httpClient);
			}
	...
	private static Map<String, OAuth2ServiceConfiguration> mapXsuaaServicePlans(
			Map<String, OAuth2ServiceConfiguration> allXsuaaServices) {
		Map<String, OAuth2ServiceConfiguration> allXsuaaServicesWithPlans = new HashMap<>();// <planName,
																							// com.sap.cloud.security.com.sap.cloud.security.config>
		if (allXsuaaServices.isEmpty()) {
			return allXsuaaServices;
		}
		Map<String, String> serviceInstancePlans = serviceManagerService.getServiceInstancePlans();// <xsuaaName,
																									// planName>
	...
	private static OAuth2ServiceConfiguration loadServiceManagerConfig(String serviceManagerPath) {
		File[] serviceBindings = new File(
				serviceManagerPath != null ? serviceManagerPath : DEFAULT_SERVICE_MANAGER_PATH).listFiles();
	...
	public OAuth2ServiceConfiguration getXsuaaConfiguration() {
		Map<String, OAuth2ServiceConfiguration> xsuaaPlans = getServiceConfigurations().get(Service.XSUAA);
		if (xsuaaPlans == null) {
			return null;
		}
	...
	public OAuth2ServiceConfiguration getXsuaaConfigurationForTokenExchange() {
		return Optional.ofNullable(getServiceConfigurations().get(Service.XSUAA).get(Plan.BROKER.name())).orElse(null);
	}

	@Nullable
	@Override
	public OAuth2ServiceConfiguration getIasConfiguration() {
		Optional<Map.Entry<String, OAuth2ServiceConfiguration>> iasConfigEntry = getServiceConfigurations()
	...
	public int getNumberOfXsuaaConfigurations() {
		return getServiceBindings(Service.XSUAA) != null ? getServiceBindings(Service.XSUAA).length : 0;
	}

	@Nullable
	static File[] getServiceBindings(Service service) {
		String path = service == Service.XSUAA ? instance.xsuaaPath : service == Service.IAS ? instance.iasPath : null;
		LOGGER.debug("Retrieving {} service bindings from K8s secret file {}", service, path);
		// TODO check whether path is not null and file exists - return null or better
		// exception?
