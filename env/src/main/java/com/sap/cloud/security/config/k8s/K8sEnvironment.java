...
public class K8sEnvironment implements Environment {
    ...
    private static Map<String, OAuth2ServiceConfiguration> mapXsuaaServicePlans(Map<String, OAuth2ServiceConfiguration> allXsuaaServices) {
        Map<String, OAuth2ServiceConfiguration> allXsuaaServicesWithPlans = new HashMap<>();//<planName, com.sap.cloud.security.com.sap.cloud.security.config>
        if (allXsuaaServices.isEmpty()){
    ...
    public OAuth2ServiceConfiguration getIasConfiguration() {
        Optional<Map.Entry<String, OAuth2ServiceConfiguration>> iasConfigEntry = getServiceConfigurations().get(Service.IAS).entrySet().stream().findFirst();
