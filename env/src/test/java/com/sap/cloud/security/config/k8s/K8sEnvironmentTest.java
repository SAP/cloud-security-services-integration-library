...
public class K8sEnvironmentTest {
    ...
    private static final WireMockServer wireMockServer = new WireMockServer(WireMockConfiguration.wireMockConfig().port(1111));
    ...
    public void init() throws IOException {
        ...
        wireMockServer.stubFor(WireMock.post("/oauth/token").willReturn(WireMock.ok().withHeader("Content-Type", "application/json").withBody("{\"access_token\": \""+ token + "\",  \"expires_in\": 1799}")));
        wireMockServer.stubFor(WireMock.get("/v1/service_plans").willReturn(WireMock.ok().withHeader("Content-Type", "application/json").withBody(servicePlans)));
        wireMockServer.stubFor(WireMock.get("/v1/service_instances").willReturn(WireMock.ok().withHeader("Content-Type", "application/json").withBody(serviceInstances)));
