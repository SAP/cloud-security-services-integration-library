...
//
// return new Jwt("InvalidMockJwtValue", Instant.now(),
...
//
// Map<String, Object> jwtClaims = new HashMap<String, Object>();
// jwtClaims.put("scope", Arrays.asList("uaa.user", "read", "write")); // got
// the uaa.user scope!
//
// return new Jwt("ValidMockJwtValue", Instant.now(),
// Instant.now().plusMillis(100000), jwtHeaders, jwtClaims);
// }
//
// @Test
...
// return new UserTokenFlow(restTemplate, refreshTokenFlowMock, new
...
//
// assertThatThrownBy(() -> {
// new UserTokenFlow(null, refreshTokenFlowMock, new
// XsuaaDefaultEndpoints(TestConstants.xsuaaBaseUri));
// }).isInstanceOf(IllegalArgumentException.class).hasMessageStartingWith("RestTemplate");
//
// assertThatThrownBy(() -> {
// new UserTokenFlow(restTemplate, null, new
// XsuaaDefaultEndpoints(TestConstants.xsuaaBaseUri));
...
//
// assertThatThrownBy(() -> {
// new UserTokenFlow(restTemplate, refreshTokenFlowMock, new
// XsuaaDefaultEndpoints(TestConstants.xsuaaBaseUri))
// .token(invalidMockJwt)
// .client(clientId)
// .secret(clientSecret)
// .execute();
// }).isInstanceOf(TokenFlowException.class).hasMessageContaining("JWT token
// does not include scope 'uaa.user'");
// }
//
// @Test
...
// .execute();
// }).isInstanceOf(TokenFlowException.class).hasMessageContaining("User token
...
//
// UserTokenFlow tokenFlow = new UserTokenFlow(restTemplateMock,
// refreshTokenFlowMock,
