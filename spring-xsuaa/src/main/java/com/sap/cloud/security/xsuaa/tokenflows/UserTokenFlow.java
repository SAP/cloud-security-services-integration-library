...
import static com.sap.cloud.security.xsuaa.tokenflows.XsuaaTokenFlowsUtils.addAuthorizationBearerHeader;
...
public class UserTokenFlow {
     ...
     * @param token - the JWT token to exchange for another.
     ...
     */
    UserTokenFlow(RestTemplate restTemplate, RefreshTokenFlow refreshTokenFlow, URI tokenEndpoint, URI authorizeEndpoint, URI keySetEndpoint) {
        Assert.notNull(restTemplate, "RestTemplate must not be null.");
        Assert.notNull(refreshTokenFlow, "RefreshTokenFlow must not be null.");
    ...
    /**
     * Sets the OAuth 2.0 client ID of the application that the exchanged 
     * token is intended for.<br>
     *  
     * <b>Note:</b> This is usually not the client ID
     * of the application that executes this flow, but that of 
     * an other application this application intends to call with the
     * exchanged token.
     *  
     * @param clientId - the OAuth 2.0 client ID of the client for which the exchanged token is intended.
     * @return this builder object.
     */
    ...
    /**
     * Sets the OAuth 2.0 client secret of the application that the exchanged
     * token is intended for.<br> 
     * 
     * <b>Note.</b> It is highly questionable that this is correct. The client secret should not be known to the application executing this flow. 
     * 
     * @param clientSecret - the secret of the OAuth 2.0 client that the exchanged token is intended for. 
     * @return this builder object.
     */
    ...
    /**
     * Executes this flow against the XSUAA endpoint. As a result the exchanged JWT token is returned. <br>
     ...
     * @throws TokenFlowException in case not all mandatory fields of the token flow request have been set.
     */
    private void checkRequest(XsuaaTokenFlowRequest request) throws TokenFlowException {
        
        if(token == null) {
            throw new TokenFlowException("User token not set. Make sure to have called the token() method on UserTokenFlow builder.");
        }
        
        boolean isUserToken = hasScope(token, UAA_USER_SCOPE);
        if (!isUserToken) {
            throw new TokenFlowException("JWT token does not include scope 'uaa.user'. Only user tokens can be exchanged for another user token.");
        }
        
        if (!request.isValid()) {
            throw new TokenFlowException("User token flow request is not valid. Make sure all mandatory fields are set.");
        }
    }
    ...
    private Jwt requestUserToken(XsuaaTokenFlowRequest request) throws TokenFlowException {
        ...
        if (responseStatusCode == HttpStatus.UNAUTHORIZED) {
            throw new TokenFlowException(String.format("Error exchanging token. Received status code %s. Call to XSUAA was not successful (grant_type: user_token). Client credentials invalid.", responseStatusCode));
        }
        
        if (responseStatusCode != HttpStatus.OK) {
            throw new TokenFlowException(String.format("Error exchanging token. Received status code %s. Call to XSUAA was not successful (grant_type: user_token).", responseEntity.getStatusCode()));
        }
        
        // Now we have a response, that contains a refresh-token. Following the standard,
        // we would now send that token to another service / OAuth 2.0 client and it would
        // there be exchanged for a new JWT token.
        // See: https://docs.cloudfoundry.org/api/uaa/version/4.31.0/index.html#user-token-grant
    ...
    /**
     * Checks if a given scope is contained inside
     * the given token.
