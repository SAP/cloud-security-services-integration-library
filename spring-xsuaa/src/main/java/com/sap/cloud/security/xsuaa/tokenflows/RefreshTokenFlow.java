...
public class RefreshTokenFlow {
     ...
     * @param refreshToken - the refresh token to be exchanged for a JWT.
     ...
     * @return the refreshed JWT token or an exception in case the token could not be refreshed.
     * @throws TokenFlowException in case of an error during the flow, or when the token cannot be refreshed.
     */
    ...
    /**
     * Checks that all mandatory fields of the token flow request have been set. Otherwise throws an exception.
    ...
    private void checkRequest(XsuaaTokenFlowRequest request) throws TokenFlowException {
        ...
        if(refreshToken == null) {
            throw new TokenFlowException("Refresh token not set. Make sure to have called the refreshToken() method on RefreshTokenFlow builder.");
        }
        
        if (!request.isValid()) {
            throw new TokenFlowException("Refresh token flow request is not valid. Make sure all mandatory fields are set.");
        }
    }
     ...
     * @param request - the token flow request to execute.
     * @return the JWT received in exchange for the refresh token.
     * @throws TokenFlowException in case of an error in the flow.
     */
    private Jwt refreshToken(String refreshToken, XsuaaTokenFlowRequest request) throws TokenFlowException {
        ...
        if (responseStatusCode == HttpStatus.UNAUTHORIZED) {
            throw new TokenFlowException(String.format("Error refreshing token. Received status code %s. Call to XSUAA was not successful (grant_type: refresh_token). Client credentials invalid.", responseStatusCode));
        }

        if (responseStatusCode != HttpStatus.OK) {
            throw new TokenFlowException(String.format("Error refreshing token. Received status code %s. Call to XSUAA was not successful (grant_type: refresh_token).", responseStatusCode));
        }
     ...
     * @throws TokenFlowException in case of a decoding error.
     */
