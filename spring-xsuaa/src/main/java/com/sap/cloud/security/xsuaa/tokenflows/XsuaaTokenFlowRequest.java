...
/**
 * An internal token exchange request capturing data 
 * by the token flow builders {@link ClientCredentialsTokenFlow} and {@link UserTokenFlow}.
 */
class XsuaaTokenFlowRequest {
	 ...
	 * @param tokenEndpoint - the endpoint URI of the XSUAA where to exchange the token.
	 */
	 ...
	 * For example {@code https://<server>:<port>/uaa/oauth/token}.
    ...
    /**
     * Returns the endpoint to authorize scopes.
     * For example {@code https://<server>:<port>/uaa/oauth/authorize}.
    ...
    /**
     * Returns the endpoint to fetch the public key set from.
     * For example {@code https://<server>:<port>/.well-known/jwks.json}.
	...
	/**
	 * Sets the requested additional authorization attributes for this token exchange
	 * request. Applications can use this to add additional authorization
	 * attributes to the exchanged access token.
	 * 
	 * @param additionalAuthorizationAttributes 
	 *        - the additional authorization attributes the exchanged token should contain.
	 */
	 ...
	 * @return {@code true}, if the request has all mandatory fields set. {@code false} otherwise.
	 */
	boolean isValid() {	    
