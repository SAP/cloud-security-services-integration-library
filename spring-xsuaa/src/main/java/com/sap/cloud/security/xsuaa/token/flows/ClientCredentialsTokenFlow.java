...
public class ClientCredentialsTokenFlow {
	 ...
	 * 			  - the token decoder
	 */
	ClientCredentialsTokenFlow(OAuth2Server oAuth2Server, VariableKeySetUriTokenDecoder tokenDecoder, OAuth2ServerEndpointsProvider endpointsProvider) {
	...
	private Jwt requestTechnicalUserToken(XSTokenRequest request) throws TokenFlowException {
		...
		try {
			OAuth2AccessToken accessToken = oAuth2Server.retrieveAccessTokenViaClientCredentialsGrant(request.getTokenEndpoint(), new ClientCredentials(request.getClientId(), request.getClientSecret()), requestParameter);
			return decode(accessToken.getValue(), endpointsProvider.getJwksUri());
		} catch (OAuth2ServerException e) {
			throw new TokenFlowException(String.format("Error requesting user token with grant_type 'client_credentials': %s",  e.getMessage()));
		}
