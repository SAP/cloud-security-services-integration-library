...
**This sample showcases the `resourceserver-security-spring-boot-3-starter`**, which provides backward compatibility for applications still using **Spring Boot 3.x** while the main library has moved to Spring Boot 4.x.
...
| Aspect | Main Starter (`resourceserver-security-spring-boot-starter`) | Spring Boot 3 Starter (`resourceserver-security-spring-boot-3-starter`) |
|--------|--------------------------------|------------------------------------------------|
...
This sample uses the `resourceserver-security-spring-boot-3-starter` to validate JWT tokens issued by either the `xsuaa` or the `identity` service.
