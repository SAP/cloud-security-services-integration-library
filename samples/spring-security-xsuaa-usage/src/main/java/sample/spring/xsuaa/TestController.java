...
public class TestController {
	...
	/**
	 * Returns some generic information from the JWT token.<br> Uses a Jwt retrieved from the security context of Spring
	 * Security.
	 *
	 * @param jwt
	 * 		the JWT from the request injected by Spring Security.
	 * @return the requested address.
	 */
	...
	/**
	 * More advanced showcase for global method security. The {@link DataService} interface uses annotated methods and
	 * when the {@link DataService} gets injected as a bean Spring Security wraps it with a security-enforcing wrapper.
	 * The result is, that the {@link DataService#readSensitiveData()} method will only be called if the proper scopes
	 * are available.
	 *
	 * @return the sensitive data read from the {@link DataService} or fails with an access denied error.
	 */
	...
	/**
	 * REST endpoint showing how to fetch a client credentials Token from XSUAA using the {@link XsuaaTokenFlows} API.
	 *
	 ...
	 * <p>
	 * The idea behind a user token exchange is to separate service-specific access scopes into separate tokens. For
	 * example, if Service A has scopes specific to its functionality and Service B has other scopes, the intention is
	 * that there is no single Jwt token that contains all of these scopes.<br> Rather the intention is to have a Jwt
	 * token to call Service A (containing just the scopes of Service A), and another one to call Service B (containing
	 * just the scopes of Service B). An application calling Service A and B on behalf of a user therefore has to
	 * exchange the user's Jwt token against a token for Service A and B respectively before calling these services.
	 * This scenario is handled by the JWT Bearer token flow.
	 * <p>
	 *
	 * @param token
	 * 		- the Jwt as a result of authentication.
	 * @throws TokenFlowException
	 * 		in case of any errors.
	 */
	 ...
	 * 		- the Jwt as a result of authentication.
	 * @param refreshToken
	 * 		- the refresh token an access token is requested
	 * @throws TokenFlowException
	 * 		in case of any errors.
	 */
	@GetMapping("/v3/requestRefreshToken/{refreshToken}")
	public String requestRefreshToken(@AuthenticationPrincipal Jwt jwt,
			@PathVariable("refreshToken") String refreshToken) throws TokenFlowException {
