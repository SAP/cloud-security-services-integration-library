...
/**
 * Tests the filter chain defined in SecurityConfiguration with calls to the {@link TestController} endpoint. Overrides
 * the default XsuaaOAuth2TokenService, that is used by the TokenBrokerResolver to fetch tokens, with a stub. The tokens
 * are generated by a {@link com.sap.cloud.security.test.JwtGenerator} and validated with a matching JWKS from the local
 * server of {@link XsuaaExtension}.
 */
@SpringBootTest
...
public class SecurityConfigurationTest {
	...
	/**
	 * Configures the xsuaaTokenService with stub responses. For each user, a generated access token is returned if the
	 * user's name, password and zone are correct. For each user, an exception is thrown when a wrong password is used.
	 * For Each user, an exception is thrown when he tries to fetch a token from the wrong zone (For the sake of
	 * testing, no user exists on more than one zone).
	 */
	@BeforeEach
		...
		if (tokens == null) {
			...
			for (Token t : tokens) {
				when(xsuaaTokenService.retrieveAccessTokenViaPasswordGrant(any(URI.class), any(ClientIdentity.class),
						eq(t.getClaimAsString(TokenClaims.USER_NAME)), eq(CORRECT_PASSWORD),
						eq(t.getClaimAsString(TokenClaims.XSUAA.ZONE_ID)), any(), anyBoolean()
				)).thenReturn(new OAuth2TokenResponse(t.getTokenValue(), Long.MAX_VALUE, null));

				when(xsuaaTokenService.retrieveAccessTokenViaPasswordGrant(any(URI.class), any(ClientIdentity.class),
						eq(t.getClaimAsString(TokenClaims.USER_NAME)), not(eq(CORRECT_PASSWORD)), any(), any(),
						anyBoolean()
				)).thenThrow(new OAuth2ServiceException("Invalid password."));

				when(xsuaaTokenService.retrieveAccessTokenViaPasswordGrant(any(URI.class), any(ClientIdentity.class),
						eq(t.getClaimAsString(TokenClaims.USER_NAME)), any(),
						not(eq(t.getClaimAsString(TokenClaims.XSUAA.ZONE_ID))), any(), anyBoolean()
